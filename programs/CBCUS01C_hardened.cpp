/**
 * CBCUS01C — CardDemo CICS Program
...
void p_1000_custfile_get_next();
void p_0000_custfile_open();
void p_9000_custfile_close();
void p_z_abend_program();
void p_z_display_io_status();

void p_1000_custfile_get_next() {
    std::cout << "START OF EXECUTION OF PROGRAM CBCUS01C" << std::endl;
    p_0000_custfile_open();
    while (true) {
        if (end_of_file.at(0) == 'Y') break;
        if (end_of_file.at(0) == 'N') {
            p_1000_custfile_get_next();
            if (end_of_file.at(0) == 'N') {
                std::cout << customer_record << std::endl;
            }
        }
    }
    p_9000_custfile_close();
    std::cout << "END OF EXECUTION OF PROGRAM CBCUS01C" << std::endl;
    // READ NEXT custfile-FILE
    if (file_status != "EXCEPTION_0") {
    } else {
        customer_record = fd_cust_id.substr(0, 500);
    }
    if (custfile_status.substr(0, 2) == "00") {
        appl_result = "0";
        std::cout << customer_record << std::endl;
    } else {
        if (custfile_status.substr(0, 2) == "10") {
            appl_result = "16";
        } else {
            appl_result = "12";
        }
    ...
    if (to_int(RETURN_CODE) == 0) {
        ...
        if (to_int(RETURN_CODE) == 16) {
            ...
            std::cout << "ERROR READING CUSTOMER FILE" << std::endl;
...
void p_z_abend_program() {
    ...
    // RETURN_CODE = ((int (*)(void *, void *))call_CEE3ABD.funcint) (b_58, b_59);
}
