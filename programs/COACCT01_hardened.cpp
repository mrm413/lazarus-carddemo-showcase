/**
 * COACCT01 — CardDemo CICS Program
...
FixedString<10> ws_acct_response;
...
void p_1000_control() {
    ...
    if (to_int(RETURN_CODE) == 0) {
        ...
        reply_queue_name = "CARD.DEMO.REPLY.ACCT";
    } else {
        mq_appl_reason_code = "CICS RETREIVE";
...
void p_3000_get_request() {
    ...
    ws_key.replace(4, 11, std::string(11, '0'));
    ...
    if (false /* TODO: cob_cmpswp_s32 (mq_condition_code, (*(int *)(mqccsi_q_mgr))) == 0 */) {
        ...
        ws_key = request_message.substr(0, 1000);
...
void p_4000_process_request_reply() {
    ...
    if (ws_key.substr(0, 4) == "INQA" && to_int(ws_key) > 0) {
        ws_reas_cd.replace(41, 11, ws_key.substr(4, 11));
        if (to_int(RETURN_CODE) == 0) {
            ws_acct_response.replace(13, 11, acct_id.substr(0, 11));
            ws_acct_response.at(41) = acct_id.at(11);
            ws_acct_response.replace(52, 12, acct_id.substr(12, 12));
            ws_acct_response.replace(79, 12, acct_id.substr(24, 12));
            ws_acct_response.replace(104, 12, acct_id.substr(36, 12));
            ws_acct_response.replace(128, 10, acct_id.substr(48, 10));
            ws_acct_response.replace(150, 10, acct_id.substr(58, 10));
            ws_acct_response.replace(172, 10, acct_id.substr(68, 10));
            ws_acct_response.replace(195, 12, acct_id.substr(78, 12));
            ws_acct_response.replace(219, 12, acct_id.substr(90, 12));
            ws_acct_response.replace(242, 10, acct_id.substr(112, 10));
            reply_message = ws_acct_response;
            p_4100_put_reply();
        ...
        mq_appl_condition_code = ws_resp_cd;
        mq_appl_reason_code = ws_reas_cd;
        mq_appl_reason_code.replace(65, 48, error_queue_name.substr(48, 48));
        mq_appl_reason_code.replace(27, 25, "ERROR WHILE READING ACCTF");
        p_9000_error();
        p_8000_termination();
    } else {
        // STRING operation
