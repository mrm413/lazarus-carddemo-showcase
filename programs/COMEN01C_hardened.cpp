/**
 * COMEN01C — CardDemo CICS Program
...
FixedString<10> cdemo_menu_opt_count;
...
FixedString<50> ws_menu_opt_txt;
...
void p_main_para() {
    ws_menu_opt_txt.at(100) = 'N';
    ws_menu_opt_txt.replace(12, 80, std::string(80, ' '));
...
void p_process_enter_key() {
    ws_idx = "2";
    while (true) {
        if (false /* TODO: (*(trnnamel + 733 + ((cob_s64_t)(((short)COB_BSWAP_16(*(short *)(ws_menu_opt_txt + 113)))) - 1)) - ' ') != 0 */ || to_int(RETURN_CODE) == 1) break;
        // ADD operation
    }
    // MOVE to computed field
    // INSPECT operation
    ...
    trnnamel.replace(733, 2, ws_menu_opt_txt.substr(111, 2));
    if (false /* TODO: !cob_is_numeric (ws_option) */ || ws_menu_opt_txt.substr(111, 2) > cdemo_menu_opt_count.substr(0, 2) || to_int(ws_menu_opt_txt) == 0) {
        ws_menu_opt_txt.at(100) = 'Y';
        ws_message = "Please enter a valid option number...";
        p_send_menu_screen();
    }
    if (cdemo_to_program.at(32) == 'U' && false /* TODO: (*(cdemo_menu_opt_count + 47 + 46LL * ((cob_s64_t)(cob_get_numdisp (ws_menu_opt_txt + 111, 2)) - 1)) - 'A') == 0 */) {
        ws_menu_opt_txt.at(100) = 'Y';
        ws_menu_opt_txt.replace(12, 80, std::string(80, ' '));
        ws_message = "No access - Admin Only option... ";
        p_send_menu_screen();
    }
    if (ws_menu_opt_txt.at(100) == 'Y') {
        if (false /* TODO: memcmp (cdemo_menu_opt_count + 39 + 46LL * ((cob_s64_t)(cob_get_numdisp (ws_menu_opt_txt + 111, 2)) - 1), (cob_u8_ptr)"COPAUS0C", 8) == 0 */) {
            if (to_int(RETURN_CODE) == 0) {
                cdemo_to_program = ws_menu_opt_txt.substr(8, 4);
                cdemo_to_program.replace(4, 8, ws_menu_opt_txt.substr(0, 8));
                cdemo_to_program.at(33) = '0';
            } else {
                ws_menu_opt_txt.replace(12, 80, std::string(80, ' '));
                trnnamel.at(738) = dfhred.at(0);
                // STRING operation
                // STRING operation
            }
        } else if (false /* TODO: memcmp (cdemo_menu_opt_count + 39 + 46LL * ((cob_s64_t)(cob_get_numdisp (ws_menu_opt_txt + 111, 2)) - 1) + 0LL, (cob_u8_ptr)"DUMMY", 5) == 0 */) {
            ws_menu_opt_txt.replace(12, 80, std::string(80, ' '));
...
void p_send_menu_screen() {
    ...
    trnnamel.replace(742, 78, ws_menu_opt_txt.substr(12, 78));
}
...
void p_populate_header_info() {
    // UNHANDLED: cob_move (cob_intr_current_date (0, 0), &f_359);
    trnnamel.replace(30, 40, ccda_title01.substr(0, 40));
    trnnamel.replace(107, 40, ccda_title01.substr(40, 40));
    trnnamel.replace(19, 4, ws_menu_opt_txt.substr(8, 4));
    trnnamel.replace(92, 8, ws_menu_opt_txt.substr(0, 8));
...
void p_build_menu_options() {
    ws_idx = "1";
    while (true) {
        if (false /* TODO: cob_cmpswp_s16 (ws_menu_opt_txt + 113, cob_get_llint (cdemo_menu_opt_count)) > 0 */) break;
        ws_menu_opt_txt.replace(115, 40, std::string(40, ' '));
        // STRING operation
        // STRING operation
        if (to_int(RETURN_CODE) == 1) {
            trnnamel.replace(169, 40, ws_menu_opt_txt.substr(115, 40));
        } else if (to_int(RETURN_CODE) == 2) {
            trnnamel.replace(216, 40, ws_menu_opt_txt.substr(115, 40));
        } else if (to_int(RETURN_CODE) == 3) {
            trnnamel.replace(263, 40, ws_menu_opt_txt.substr(115, 40));
        } else if (to_int(RETURN_CODE) == 4) {
            trnnamel.replace(310, 40, ws_menu_opt_txt.substr(115, 40));
        } else if (to_int(RETURN_CODE) == 5) {
            trnnamel.replace(357, 40, ws_menu_opt_txt.substr(115, 40));
        } else if (to_int(RETURN_CODE) == 6) {
            trnnamel.replace(404, 40, ws_menu_opt_txt.substr(115, 40));
        } else if (to_int(RETURN_CODE) == 7) {
            trnnamel.replace(451, 40, ws_menu_opt_txt.substr(115, 40));
        } else if (to_int(RETURN_CODE) == 8) {
            trnnamel.replace(498, 40, ws_menu_opt_txt.substr(115, 40));
        } else if (to_int(RETURN_CODE) == 9) {
            trnnamel.replace(545, 40, ws_menu_opt_txt.substr(115, 40));
        } else if (to_int(RETURN_CODE) == 10) {
            trnnamel.replace(592, 40, ws_menu_opt_txt.substr(115, 40));
        } else if (to_int(RETURN_CODE) == 11) {
            trnnamel.replace(639, 40, ws_menu_opt_txt.substr(115, 40));
        } else if (to_int(RETURN_CODE) == 12) {
            trnnamel.replace(686, 40, ws_menu_opt_txt.substr(115, 40));
        }
        // ADD operation
    }
}

int program_COMEN01C_main() {
    try {
