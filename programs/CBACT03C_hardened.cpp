/**
 * CBACT03C — CardDemo CICS Program
...
void p_0000_xreffile_open();
void p_9000_xreffile_close();
void p_9999_abend_program();
void p_9910_display_io_status();

void p_1000_xreffile_get_next() {
    std::cout << "START OF EXECUTION OF PROGRAM CBACT03C" << std::endl;
    p_0000_xreffile_open();
    while (true) {
        if (end_of_file.at(0) == 'Y') break;
        if (end_of_file.at(0) == 'N') {
            p_1000_xreffile_get_next();
            if (end_of_file.at(0) == 'N') {
                std::cout << card_xref_record << std::endl;
            }
        }
    }
    p_9000_xreffile_close();
    std::cout << "END OF EXECUTION OF PROGRAM CBACT03C" << std::endl;
    // READ NEXT xreffile-FILE
    if (file_status != "EXCEPTION_0") {
    } else {
        card_xref_record = fd_xref_card_num.substr(0, 50);
    }
    if (xreffile_status.substr(0, 2) == "00") {
        appl_result = "0";
        std::cout << card_xref_record << std::endl;
    } else {
        if (xreffile_status.substr(0, 2) == "10") {
            appl_result = "16";
        } else {
            appl_result = "12";
        }
...
void p_9999_abend_program() {
    ...
    // RETURN_CODE = ((int (*)(void *, void *))call_CEE3ABD.funcint) (b_43, b_44);
}
